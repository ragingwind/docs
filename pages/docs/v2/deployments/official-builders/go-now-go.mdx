...
export const meta = {
  ...
  lastEdited: '2019-05-20T10:16:58.000Z'
}
...
To use private packages with `go get`, define git credential as [build environment variable](/docs/v2/deployments/configuration#build-env) in `now.json` as follows:
...
We support major Git providers GitHub, GitLab, Bitbucket, and self-host Git server.

<Note>
  <InlineCode>GIT_HOST</InlineCode> expected value without protocol. We use
  `HTTPS` by default.
</Note>
